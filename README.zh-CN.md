...
- **主动流量整形**：全生命周期 Markov 状态机（TrafficShaper）主动对每条应用数据（0x17）记录进行切分、填充和节拍控制，记录线速尺寸由 shaper 策略决定——明文长度不再映射至线速尺寸。支持可选的 Restls 风格声明式剧本（`traffic_script`）对握手后包序列进行确定性控制，包括记录间 Delay 时序（对数正态或预录制 IAT 回放）与非对称 FakeResponse 交互（CMD_PADDING）。所有填充字节源自统一的 8 MiB CSPRNG 预种子噪声池，与真实 AEAD 密文密码学同构。
...
| `settings.session.traffic_script`          | 双方 | 可选 Restls 风格流量剧本（参见 docs/MECHANISM.zh-CN.md §3.5） |
...
| `settings.session.traffic_script`          | 可选 Restls 风格流量剧本（参见 docs/MECHANISM.zh-CN.md §3.5） |
