...
| `settings.session.traffic_script`          | 双方 | 可选声明式流量脚本（参见 docs/MECHANISM.zh-CN.md §3.5） |
...
| `settings.tls.insecure` | 跳过 rustls 路径 TLS 证书验证（默认 `false`）。仅影响原生 rustls ClientHello 生成；Noise 提供端点认证。 |
...
| `settings.session.idle_timeout_secs` | session 空闲超时（默认 45，客户端侧 clamp 至 [5, 3600]） |
| `settings.session.max_streams_per_session` | 单 session 最大并发 stream 数（默认 256，验证至 [1, 4096]） |
| `settings.session.traffic_script` | 可选声明式流量剧本（参见 docs/MECHANISM.zh-CN.md §3.5） |
