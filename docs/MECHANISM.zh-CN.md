...
| bad_record_mac 告警 | 2（`[02 14]`） | 18 | **23** | — |
...
当收到的 0x17 记录 Noise AEAD 解密失败（`read_message` 返回 `Err`）时，隧道**不会**发送正常的 `close_notify`。取而代之：

1. 构造 TLS 1.3 致命告警 `bad_record_mac`（0x02, 0x14）并使用 Noise 加密。
2. 告警封装为 0x17 记录并排入写缓冲区。
3. 在 TCP socket 上设置 `SO_LINGER = 0`（关闭时强制 RST）。
4. 将 `close_notify_written` 设为 `true`，状态设为 `Closed`（防止正常的 `close_notify` 被发送）。
5. 返回 IO 错误，触发 session 拆除和 TCP RST。
