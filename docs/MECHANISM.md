...
Every hardcoded value must therefore name a real source: Firefox's keepalive 600 s / 1 s / 4 probes, nginx's `keepalive_time 1h`, Firefox's `ping-threshold = 58 s`, Firefox's `kInitialRwin = 12 MiB` (halved for the WINDOW_UPDATE threshold), the H2 SETTINGS / WINDOW_UPDATE / PING / GOAWAY frame sizes, and the 15 extensions of the captured Firefox ClientHello. **Global uniformity is not a defect here, provided what we are uniform with is real.**
